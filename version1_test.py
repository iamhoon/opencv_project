...
class Player():
    ...
    def calculation_between_base(self):
        ...
        if (math.sqrt(pow(self.next_base[0] - self.nowPoint[0], 2) + pow(self.next_base[1] - self.nowPoint[1], 2))) < 10.0:
            print("---------베이스 통과-----------")
    ...
    # 초기 베이스 위치 설정 함수 초기 위치에서 가장 가까운 위치의 베이스를 초기 위치로 잡는다.
    def set_base(self):
        base_length = {
            "거리" : [],
            "베이스": [],
    ...
    def player_data_box(self):
        ...
        if self.route_pointList != []:
            ...
            # cv2.line(img, (int(int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) * 1 / 16), 0), (int(int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) * 1 / 16), 90), (255, 255, 255), 2)
...
while True:
    ...
    background_mask = background.apply(img)
    ...
    for i in range(len(contours)):
        cv2.drawContours(background_mask, contours, -1, (255, 0, 0), 1)

    cv2.imshow("back", background_mask)
